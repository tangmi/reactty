...
Viewport.prototype.lines = function(text) {
	...
	console.log(x, y, w, h, this._numColumns, this._numRows, outLines.length);
	...
	for (var i = 0; i < srcLines.length; i++) {
		...
		// add strings into the "outLines" buffer showing only what's visible
		if (i >= y - 1 && i < y - 1 + h - 1) {
			if (i < srcLines.length) {
				// if (!_config.wrap) {
				outLines.push(line.substr(x - 1, w));
				// } else {
				// 	outLines.push(line);
				// 	x = 1;

				// 	// increment the number of rows if we have wrapping
				// 	if (line.length >= w) {
				// 		numRows++;
				// 	}
				// }
	...
	for (var i = outLines.length; i < h - 1; i++) {
		outLines.push('%');
	}
